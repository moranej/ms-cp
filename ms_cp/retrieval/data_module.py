...
class MassSpecDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if not set(self.split) <= {"train", "val", "test", "calib"}:
            raise ValueError(
                '"Folds" column must contain only "train", "val", "test", or "calib" values.'
            )
        ...
        if stage == "fit" or stage is None:
            ...
            self.calib_dataset = Subset(self.dataset, np.where(split_mask == "calib")[0])
        if stage == "test":
